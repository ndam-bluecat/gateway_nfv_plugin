...
* Table overview of IPAddress configuration for each services in **docker-compose.yml**:
    ...
    | `bluecat_gateway` | 192.0.2.13 | 2001:DB8::2001:DB8:0:3 |
...
2. Configure **Gateway Container**:
    ...
    | `enviroment` | Enviroment of container includes BAM_IP and LOCAL_USER_ID |
    | `volumes` | config gateway directory and logs want to mount here |

> Note: Remembar to **BAM_IP**, **LOCAL_USER_ID** and **PATH OF NFV_GATEWAY** 

3. Configure Memcached container

    ![DC Memcache](images/docker_compose_memcache.png?raw=true)
...
4. Configure Scheduler Statistic Collection Container
    ...
    | `ipv4_address` | IPv4Address  of container |
    | `ipv6_address` | IPv6Address  of container |
    | `volumes` | Configure folder and logs want to mount here |

## Setup Gateway Container

1. Copy the gateway **.tar** file to the host machine

2. From the Linux console, run the following command:

    ```bash
    docker load -i <path_to_image_tar_file>
    ```

3. Set the permissions to external volumes with the **data** and **logs** directories:

    ```bash
    chmod -R o=rwx <mapped volume>
    ```

## Build docker image Scheduler Statistic Collection
- Note:  Using when you want to change code scheduler and rebuild it or not have **gateway_nfv_scheduler.tar**.
...
4. Load **memcached.tar** and **gateway_nfv_scheduler.tar** files before running the memcached and scheduler

    ```bash
    docker load -i <path_to_tar_file>
    ```

5. Get ip for the container(s)
...
1. Execute this command to install 3rd Python libraries with correct path of gateway and gateway-nfv-plugin workflow directories:
    ...
    ```bash
    docker exec bluecat_gateway pip install --target /bluecat_gateway/customizations/integrations -r path/to/gateway-nfv-plugin/workflow/requirements.txt
    ```

2. Access to Gateway Web UI. In the left sidebar, navigate to **Administration** and **Encrypt Password** action. Input the path `workflows/gateway_nfv_plugin/config/.secret` set in `config.ini` and the password of `user_name` user.

    ![Encrypt Password](images/gateway_encrypt_pwd.png?raw=true)

> Note: Ports of Gateway Web configured in the **docker-compose.yml** above
...
3. Navigate to **Workflow Permission**. Add permission to access traffic steering workflow.
    ...
    If the permission for gateway_nfv_plugin disappears, restart the gateway by the command `docker restart bluecat_gateway`.
